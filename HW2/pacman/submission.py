...
def betterEvaluationFunction(gameState):
  ...
  """
    ...
    # find closest ghost:
    closestEnemyGhost = ClosestEnemyGhostToPacman(gameState)
    closesetScaredGhost = ClosestScaredGhostToPacman(gameState)
    ...
    # closeset enemy ghost
    closesetEnemyGhostDist = numpy.inf if closestEnemyGhost is None \
        else manhattanDistance(closestEnemyGhost.configuration.pos, pacmanPos)

    # closest scared ghost
    closesetScaredGhostDist = numpy.inf if closesetScaredGhost is None \
        else manhattanDistance(closesetScaredGhost.configuration.pos, pacmanPos)

    # distance from closest food:
    listOfFood = getListOfFood(gameState)
    closestFood = getClosestElementToPacman(gameState, listOfFood) if len(listOfFood) != 0 else None
    distanceFromClosestFood = 0 if closestFood is None else manhattanDistance(closestFood, pacmanPos)
    ...
    distanceFromClosestCapsule = 0 if len(capsulesPos) == 0 \
        else manhattanDistance(getClosestElementToPacman(gameState, capsulesPos), pacmanPos)
    ...
    if closesetEnemyGhostDist < dangerRadius:
        # Danger mode
        score = 1 * gameScore \
                - 100 * (1 / (closesetEnemyGhostDist + 1))\
                + 0.01 * (1 / (closesetScaredGhostDist + 1))\
                + 0.01 * (1 / (distanceFromClosestFood + 1)) \
                + 0.1 * (1 / (distanceFromClosestCapsule + 1)) \
                + 0.01 * (1 / (foodAmount + 1))
    else:
        # safe mode
        score = 1 * gameScore \
                - 0.01 * (1 / (closesetEnemyGhostDist + 1))\
                + 0.9 * (1 / (closesetScaredGhostDist + 1))\
                + 0.1 * (1 / (distanceFromClosestFood + 1)) \
                + 0.01 * (1 / (distanceFromClosestCapsule + 1)) \
                + 0.1 * (1 / (foodAmount + 1))
...
def ClosestEnemyGhostToPacman(gameState):
    pacmanPos = gameState.getPacmanPosition()
    ...
    for ghostState in ghostStates:
        if ghostState.scaredTimer == 0:
            ...
            closestGhostPos = (numpy.inf, numpy.inf) if closestGhost is None else closestGhost.configuration.pos
            if manhattanDistance(ghostPos, pacmanPos) < manhattanDistance(closestGhostPos, pacmanPos):
                closestGhost = ghostState

    return closestGhost if closestGhost is not None else None


def ClosestScaredGhostToPacman(gameState):
    pacmanPos = gameState.getPacmanPosition()
    ghostStates = gameState.getGhostStates()
    closestGhost = None
    ...
    def minMaxRecursion(self, depth, agent_idx, game_state):
        ...
        if agent_idx == 0:  # pacman
            ...
            for action in legal_actions:
                total_sum += self.minMaxRecursion(depth, next_agent_idx,
                                                  game_state.generateSuccessor(agent_idx, action))
  ...
  """
    ...
    def minMaxRecursion(self, depth, agent_idx, game_state):
        ...
        if agent_idx == 0:  # pacman
            ...
            for action in legal_actions:
                total_sum += p[action] * self.minMaxRecursion(depth, next_agent_idx,
                                                              game_state.generateSuccessor(agent_idx, action))
            return total_sum
